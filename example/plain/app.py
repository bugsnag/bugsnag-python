...
# reporting configured in your Python code.
# ***********************************************************
...
def callback(notification):
    ...
    # is 'SpecificError', as in crash_with_callback(), below.
    if isinstance(notification.exception, SpecificError):
        tab = {
            'message': 'That\'s not how this works',
            'code': 500,
            'password': 'ue$hs_9gFsd!kjl41___',  # this will be redacted
        }
        notification.add_tab('Diagnostics', tab)
        notification.context = (
            'Check the \'Diagnostics\' tab attached only to SpecificErrors'
        )
...
def crash_callback():
    ...
    attached by the before_notify() callback above, and crash the app.
    """
...
def handle_zero_div():
    ...
    print(
        'Check https://app.bugsnag.com to view notifications'
    )


if __name__ == '__main__':
    # automatically sends this manual notification every time the file is
    # loaded.
    ...
    or, just write your own crashing statements to see how Bugsnag reports
    them!
    """
    )
